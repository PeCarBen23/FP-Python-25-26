...
def comprobar_letra(palabra_secreta:str, letra:str)->bool:
    ...
    def mostrar_mensaje (acierto:bool)->None:
    '''
    Mostrar mensaje:
...
```python
def comprobar_palabra_completa(palabra_secreta:str, letras_probadas:list[str])->bool:
    '''
...
def ejecutar_turno(palabra_secreta:str, letras_probadas:set[str])->bool:
    ...
    - Comprobar si la letra está en la palabra (True) o no (False)
    - Mostrar mensaje segun el resultado de la comprobación indicando acierto/fallo
...
def jugar(max_intentos:int, palabras:list[str])->None:
    ...
    - Copia el código que figura a continuación y 
    - completa la condición de la sentencia while para que mientras no agote los intentos y no se ha compelado la palabra se ejecute el siguiente bloque:
      > ejecutar un turno de juego
      > si acierta en el turno comprobar si se ha completado la palabra.
      > si no acierta mostrar un mensaje con los intentos que quedan y sumar 1 a los intentos
